...
Function `pracma::trapz()` calculates a trapezoidal integration (@eq-trapz), visualized in @fig-vtrapz.
...
@fig-vtrapz visualizes the concept of (cumulative) average vertical height of a trapezoidal integration using our toy example. <!--[Say some English as a summary statement.]{style="background-color: #FFFF00"}-->

```{r}
#| label: fig-vtrapz
#| code-fold: true
#| code-summary: "R code: visualize `vtrapz()` and `cumvtrapz()`, a toy example"  
#| fig-height: 3
#| fig-width: 5.5
#| fig-align: left
#| fig-cap: "(Cumulative) average vertical height; a toy example"
visualize_vtrapz(x, y) + 
  ggplot2::theme_minimal()
```


@fig-vtrapz-E visualizes the cumulative average vertical height of the trapezoidal integration for function `spatstat.explore::Emark()`.  [Say some English]{style="background-color: #FFFF00"}

```{r}
#| label: fig-vtrapz-E
#| code-fold: true
#| code-summary: "R code: visualize `cumvtrapz()` for `spatstat.explore::Emark()`"  
#| fig-height: 3
#| fig-width: 5.5
#| fig-align: left
#| fig-cap: "Cumulative average vertical height for `spatstat.explore::Emark()`"
spatstat.data::spruces |>
  spatstat.explore::Emark() |>
  visualize_vtrapz.fv() + ggplot2::theme_minimal()
```

@fig-vtrapz-V visualizes the cumulative average vertical height of the trapezoidal integration for function `spatstat.explore::Vmark()`.  [Say some English]{style="background-color: #FFFF00"}


```{r}
#| label: fig-vtrapz-V
#| code-fold: true
#| code-summary: "R code: visualize `cumvtrapz()` for `spatstat.explore::Vmark()`"  
#| fig-height: 3
#| fig-width: 5.5
#| fig-align: left
#| fig-cap: "Cumulative average vertical height for `spatstat.explore::Vmark()`"
spatstat.data::spruces |>
...
Functions `vtrapz.fv()` and `cumvtrapz.fv()` calculate the (cumulative) average vertical height of the trapezoidal integration (see Appendix @sec-vtrapz) under the black-solid-curve. They are both "psuedo" `S3` method dispatches, as the workhorse functions `vtrapz()` and `cumvtrapz()` are not `S3` generic functions as of package **`groupedHyperframe`** v`r packageVersion('groupedHyperframe')`.
