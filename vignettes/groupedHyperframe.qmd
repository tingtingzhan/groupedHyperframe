...
|------------------------|------------------------|------------------------|
| `Emark_()` | [`Emark`](https://search.r-project.org/CRAN/refmans/spatstat.explore/html/Emark.html), conditional mean, diagnostics for dependence between the points and the marks [@Emark] | `.E` |
| `Vmark_()` | [`Vmark`](https://search.r-project.org/CRAN/refmans/spatstat.explore/html/Vmark.html), conditional variance, diagnostics for dependence between the points and the marks [@Emark] | `.V` |
| `markcorr_()` | [`markcorr`](https://search.r-project.org/CRAN/refmans/spatstat.explore/html/markcorr.html), marked correlation function [@markcorr] | `.k` |
| `markvario_()` | [`markvario`](https://search.r-project.org/CRAN/refmans/spatstat.explore/html/markvario.html), mark variogram [@markvario] | `.gamma` |
...
-   `0`, from $0/\delta$, where $\delta$ is a positive `double`-precision scalar $\geq$ (approximately) `2.6e-324`
-   `NaN`, from $0/\varepsilon$, where $\varepsilon$ is a positive `double`-precision scalar $\leq$ (approximately) `2.5e-324`
-   `Inf`, from $\delta/0$, where $\delta$ is a positive `double`-precision scalar $\geq$ (approximately) `2.6e-324`
...
In the batch process `markcorr_()`, a same argument of `r` is used for *all* `ppp.object`s in the `ppp`-hypercolumn. Inevitably, for some `ppp.object`s the returned `fv.object`s may have exceptional values. We discuss this exception handling in the next section *Aggregation over nested grouping structure*.

#### Batch processes applicable to `multitype` `marks`

| Batch Process | Workhorse in package **`spatstat.explore`** | `fv`-hypercolumns Suffix |
|------------------------|------------------------|------------------------|
| `Gcross_()` | [`Gcross`](https://search.r-project.org/CRAN/refmans/spatstat.explore/html/Gcross.html), multitype nearest-neighbour distance function | `.G` |
| `Kcross_()` | [`Kcross`](https://search.r-project.org/CRAN/refmans/spatstat.explore/html/Kcross.html), multitype $K$-function | `.K` |
| `Jcross_()` | [`Jcross`](https://search.r-project.org/CRAN/refmans/spatstat.explore/html/Jcross.html), multitype $J$-function [@Jcross] | `.J` |
| `Lcross_()` | [`Lcross`](https://search.r-project.org/CRAN/refmans/spatstat.explore/html/Lcross.html), multitype $L$-function | `.L` |
...
|------------------|------------------|------------------|------------------|
| `nncross_()` | [`nncross.ppp`](https://search.r-project.org/CRAN/refmans/spatstat.geom/html/nncross.html)`(., what = 'dist')` | `multitype` marks | `.nncross` |
...
As we have mentioned in the previous section *Batch processes*, a same argument of `r` is used for *all* `ppp.object`s in the `ppp`-hypercolumn. Suppose a naive user uses an $r$-vector well beyond the *recommended range* and/or *available range*. In this case, function `aggregate_fv()` prints a `message` of "please limit $r$" on a ***recommended batch range***.

```{r}
r = seq.int(from = 0, to = 2000, by = 50)
no_print = s |>
  Emark_(r = r, correction = 'none') |>
  aggregate_fv(by = ~ patient_id, f_aggr_ = pmean)
```

The recommended batch range is determined by the last value of $r$, that no value of `NaN` and/or `Inf` appears in any of the `fv.object`s of the `fv`-hypercolumn, e.g., *`hladr.E`*.  Note that the `0`-values in an `fv.object` are typically a sign of degeneration as well, but function `aggregate_fv()` does **not** eliminate `0`-values from the determination of recommended batch range.
