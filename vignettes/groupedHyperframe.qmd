...
as well as @Yi25; @Yi23a; @Yi23b, we worked with a non-spatial `groupedHyperframe` object *`Ki67g`* with a `numeric`-hypercolumn *`logKi67`*, i.e., the log-transformed Ki-67 intensities, and a nested grouping structure *`~patientID/tissueID`*.

```{r}
#| code-fold: true
#| code-summary: 'A non-spatial `groupedHyperframe` *`Ki67g`*'
...
We aggregated the `quantile`s of the `numeric`-hypercolumn *`logKi67`* at the second-lowest group `~patientID`.
