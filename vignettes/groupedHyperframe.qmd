...
We present an `S3` generic function `rmax_()` which provides the *Recommended* $r_\text{max}$ based on

-   function `spatstat.explore::markcorr()`, in `S3` method dispatch `rmax_.ppp(, fun = 'K')`

Together with the `S3` method dispatches `rmax_.ppplist()` and `rmax_.hyperframe()`, user may obtain the *Recommended* $r_\text{max}$ *before* applying the very slow batch processes `Emark_()`, `Vmark_()`, `markcorr_()` and `markvario_()`.
