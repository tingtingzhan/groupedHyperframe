...
We create a `hyperframe` *`fluM1`* which consists of the same columns as *`fluM`*, but a `ppp`-hypercolumn *`pattern`* with *`M1`* marks only.  Similarly, we create another `hyperframe` *`fluM2`* which consists of the *`M2`* marks only.
...
User may recreate the hyperframe *`fluM`* by combining `hyperframe`s *`fluM2`* and *`fluM1`* using the `S3` method dispatch `superimpose.hyperframe()`.  Note that the order of *`fluM2`*-then-*`fluM1`* matters, because in *`fluM`* the points are arranged by *`M2`*-then-*`M1`*.
