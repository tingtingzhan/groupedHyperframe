...
Exceptional/illegal values `0`, `NaN` and `Inf` may appear in the mark correlation function $k_f(r)$, if $r$ goes beyond the recommended and/or available range.
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: function `summary.fvlist()`, truncated at $r_\text{max}>$ minimum Legal $r_\text{max}$ of all `fv.object`s'
m_G_s1 = m_G |>
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: function `summary.fvlist()`, truncated at $r_\text{max}\leq$ minimum Legal $r_\text{max}$ of all `fv.object`s'
m_G_s2 = m_G |>
