...
$$
\begin{split}
\dfrac{\displaystyle\int_a^b f(x)\,dx}{b-a} & \approx (b-a)^{-1} \sum_{k=1}^{N}{\dfrac{f(x_{k-1})+f(x_{k})}{2}}\Delta x_{k} \\
...
(A) our toy example; 
(B) kernel `stats::density()`, using element `$x` and `$y`;
(C) empirical cumulative distribution function `stats::ecdf()`, using the objects `x` and `y` in the enclosing `environment`;
(D) conditional mean $E(r)$ between the points and the marks, from function `spatstat.explore::Emark()`.  [Say some English]{style="background-color: #FFFF00"}
(E) conditional variance $V(r)$ between the points and the marks, from function `spatstat.explore::Vmark()`.  [Say some English]{style="background-color: #FFFF00"}
