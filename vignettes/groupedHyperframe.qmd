...
as well as @Yi25; @Yi23a; @Yi23b, was featured with a hyper data frame *`Ki67q`* with a numeric-hypercolumn *`logKi67.quantile`*. Functions in the R code-chunk below are explained in @sec-groupedHyperframe.
...
The pipeline `. |> quantile() |> aggregate()` (@sec-summary_quantile, @sec-aggregate) aggregates the `quantile`s of each element in the numeric-hypercolumn, e.g., *`hladr`* in *`wrobel_lung0g`*, by point-wise means (default of parameter `f_aggr_`) at the biologically independent group, e.g., *`~patient_id`*. The returned object is a `hyperframe` instead of a `groupedHyperframe`, as we have *one* aggregated *`hladr.quantile`* per *`~patient_id`*, thus eliminates the need for a grouping structure. The R console output is taken care of by the `S3` method dispatch `spatstat.geom::print.hyperframe()`.
...
## Summarization {#sec-summary}

### of `fv`-`hypercolumns` {#sec-summary_fv}

Function `summary_fv()` runs the workhorse function `summary.fvlist()` (Appendix @sec-summary_fvlist) across all `fv`-hypercolumns of the input `hyperframe` and/or `groupedHyperframe`. Function `summary_fv()` appends additional `numeric`-hypercolumns to the input,

-   *`<mark>.<fv>.y`*, the *function values*, i.e., the black-solid-curve (Appendix @sec-blackSolidCurve);
...
When a user-specified $r$-`vector` is provided to a batch process (@sec-batch) on *all* `ppp.object`s in the `ppp`-hypercolumns, inevitably some of the `fv`-returns may contain exceptional/illegal function values (Appendix @sec-legal_rmax). In this case, the summary is *truncated* at the minimum legal $r_\text{max}$ (Appendix @sec-legal_rmax) of the `fv.object`s in each `fv`-hypercolumn.

In the following example, the returned `groupedHyperframe` object *`out_fv`* has
...
```{r}
#| code-fold: true
#| code-summary: 'A `groupedHyperframe` object *`out_fv`*: summarizing `fv`-hypercolumns'
out_fv
```

User may specify an $r_\text{max}$ for truncation using the argument `rmax`. If the user-specified $r_\text{max}$ is greater than the minimum legal $r_\text{max}$, the user's $r_\text{max}$ will be ignored (Appendix @sec-summary_fvlist).
...
The S3 method dispatch `t.vectorlist()` is the fastest way to extract a "slice" from a `numeric`-hypercolumn, e.g., *`out_fv$hladr.E.cumvtrapz`* (Appendix @sec-vectorlist).

### of `quantile`s {#sec-summary_quantile}

The `S3` method dispatch `quantile.hyperframe()` (Appendix @sec-quantile-hyperframe) appends the additional `numeric`-hypercolumns to the input `hyperframe` and/or `groupedHyperframe`, e.g., in the example below,

-   numeric-hypercolumn *`phenotype.nncross.quantile`*, the `quantile`s of the numeric-hypercolumn *`phenotype.nncross`*.
-   numeric-hypercolumn *`hladr.quantile`*, the `quantile`s of the numeric mark *`hladr`* in the `ppp`-hypercolumn.
...
```{r}
#| code-fold: true
#| code-summary: 'A `groupedHyperframe` object *`out_q`*: summarizing `quantile`s'
out_q
```

### of Kernel `density`s {#sec-summary_kerndens}

The `S3` method dispatch `kerndens.hyperframe()` (Appendix @sec-kerndens-hyperframe) appends the additional `numeric`-hypercolumns to the input `hyperframe` and/or `groupedHyperframe`, e.g., in the example below,

-   numeric-hypercolumn *`phenotype.nncross.kerndens`*, the aggregated kernel densities of the numeric-hypercolumn *`phenotype.nncross`*.
-   numeric-hypercolumn *`hladr.kerndens`*, the aggregated kernel densities of the numeric mark *`hladr`* in the `ppp`-hypercolumn.
...
```{r}
#| code-fold: true
#| code-summary: 'A `groupedHyperframe` object *`out_k`*: summarizing kernel `density`s'
out_k
```

## Aggregation {#sec-aggregate}

The `S3` method dispatch `aggregate.groupedHyperframe()` (Appendix @sec-aggregate_groupedHyperframe) aggregates the information obtained from @sec-summary.

Example below aggregates the summarized information from `fv`-hypercolumns (@sec-summary_fv),
...
Example below aggregates the `quantile`s from numeric-hypercolumns or numeric-marks of `ppp`-hypercolumn (@sec-summary_quantile),
...
Example below aggregates the kernel `density`s from numeric-hypercolumns or numeric-marks of `ppp`-hypercolumn (@sec-summary_kerndens),
...
The authors are working on a manuscript about the batch process (@sec-batch) and the truncated aggregation (@sec-aggregate, Appendix @sec-legal_rmax, Appendix @sec-fvlist).
...
The Appendix, other than @sec-trapz, is organized in the fashion that

-   [Markdown](https://www.markdownguide.org/cheat-sheet/) *Header 2*, e.g. @sec-appendix `.0`, is about the object `class`;
-   Markdown *Header 3*, e.g. @sec-appendix `.0.0`, is about the `S3` method dispatches.
...
-   **Vignette**, to create tables in this vignette.
...
The [chained trapezoidal rule](https://en.wikipedia.org/wiki/Trapezoidal_rule) of numerically approximating a definite integral is an ancient idea since the dawn of human civilization.

Let $\{x_k; k=0,\cdots,N\}$ be a partition of the real interval $[a,b]$ such that $a = x_0 < x_1 < \cdots < x_{N-1} < x_N = b$ and $\Delta x_k= x_k - x_{k-1}$ be the length of the $k$-th sub-interval, then the chained-trapezoidal approximation of the integration is,
...
Note that the authors intentionally make the toy example data *`x`* *not* equidistant, as neither the chained trapezoidal rule framework nor the functions `pracma::trapz()` and `pracma::cumtrapz()` *require* the equidistant assumption.
...
The *average vertical height* of a trapezoidal integration is the chained-trapezoid (@eq-trapz) divided by the length of $x$-domain $b-a$,
...
Function `spatstat.geom::ppp()` creates a two-dimensional point pattern object (`ppp.object`), i.e., an R object of `S3` class `'ppp'`. In addition to the existing `S3` method dispatches to the class `'ppp'` in packages **`spatstat.geom`** and **`spatstat.explore`**,

```{r}
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches `spatstat.geom::*.ppp`'
library(spatstat.geom)
methods(class = 'ppp') |> 
...
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches `spatstat.explore::*.ppp`'
...
Package **`groupedHyperframe`** implements more `S3` method dispatches to the class `'ppp'` (@tbl-S3_ppp),

```{r}
#| code-fold: true
#| code-summary: '**Vignette**: `S3` method dispatches `groupedHyperframe::*.ppp`'
#| label: tbl-S3_ppp
...
S3_ppp |> 
  knitr::kable(
    caption = '`S3` method dispatches `groupedHyperframe::*.ppp`'
  )
```

<details><summary>@tbl-S3_ppp_no: `S3` method dispatches currently [**not**]{style="background-color: #FFFF00"} planned for class `'ppp'`</summary>

| Not Planned | Explained in |
|:-----------------|:--------------|
| ~~`na.fail.ppp()`~~, ~~`na.pass.ppp()`~~ | Appendix @sec-dont-na-ppp |

: S3 method dispatches currently not planned for class `'ppp'` {#tbl-S3_ppp_no}
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: function `log.ppp()` on numeric-`marks` in `'vector'`-`markformat`"
spatstat.data::longleaf
spatstat.data::longleaf |>
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: function `log1p.ppp()` on numeric-`marks` in `'vector'`-`markformat`"
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: function `log.ppp()` on numeric-`marks` in `'dataframe'`-`markformat`"
spatstat.data::betacells
spatstat.data::betacells |>
  spatstat.geom::marks.ppp() |>
...
### Kernel Density of `numeric`-`marks` {#sec-kerndens_ppp}

The `S3` generic function `kerndens()` extracts the `$y` element of a `density` object, i.e., `stats::density.default()$y`.

The `S3` method dispatch `kerndens.ppp()` finds the kernel `density`s of one or more `numeric` `marks` of a `ppp.object`. Note that this is not `spatstat.explore::density.ppp()`!

```{r}
#| code-fold: true
#| code-summary: "**Example**: function `kerndens.ppp()` on `'none'`-`markformat`"
spatstat.data::cells
spatstat.data::cells |>
  kerndens() |>
  is.null()
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: function `kerndens.ppp()` on numeric-`marks` in `'vector'`-`markformat`"
spatstat.data::longleaf
spatstat.data::longleaf |>
  kerndens(n = 8L)
```
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: function `kerndens.ppp()` on `multitype`-`marks` in `'vector'`-`markformat`"
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: function `kerndens.ppp()` on numeric-`marks` in `'dataframe'`-`markformat`"
spatstat.data::betacells
spatstat.data::betacells |>
  kerndens(n = 8L)
```

### Quantile of `numeric`-`marks` {#sec-quantile_ppp}

The `S3` method dispatch `quantile.ppp()` finds the `quantile`s of one or more `numeric` `marks` of a `ppp.object`.

```{r}
#| code-fold: true
#| code-summary: "**Example**: function `quantile.ppp()` on `'none'`-`markformat`"
spatstat.data::cells
spatstat.data::cells |>
  quantile() |>
  is.null()
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: function `quantile.ppp()` on numeric-`marks` in `'vector'`-`markformat`"
spatstat.data::longleaf
spatstat.data::longleaf |>
  quantile()
```
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: function `quantile.ppp()` on `multitype`-`marks` in `'vector'`-`markformat`"
spatstat.data::amacrine
spatstat.data::amacrine |>
  quantile() |>
  is.null()
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: function `quantile.ppp()` on numeric-`marks` in `'dataframe'`-`markformat`"
spatstat.data::betacells
spatstat.data::betacells |>
  quantile()
```
...
The `S3` generic function `split_kmeans()` splits `ppp.object`, `ppplist` (Appendix @sec-split_kmeans-ppplist), and `hyperframe` (Appendix @sec-split_kmeans-hyperframe) by $k$-means clustering. The authors use a `subset` of the data set *`flu`* from package **`spatstat.data`** to illustrate the various `S3` method dispatches of `split_kmeans()`.
...
The `S3` generic function `pairwise_cor_spatial()` calculates the nonparametric, rank-based, Tjøstheim's correlation coefficients [@Tjostheim78; @Hubert82] in a pairwise-combination fashion, using the workhorse function `SpatialPack::cor.spatial()` from package **`SpatialPack`** [@SpatialPack].
...
The `S3` method dispatch `pairwise_cor_spatial.ppp()` finds the nonparametric Tjøstheim's correlation coefficients from the pairwise-combinations of all numeric `marks` in a `ppp.object`.
...
The `S3` method dispatch `as.matrix.pairwise_cor_spatial()` returns a `matrix` of pairwise Tjøstheim's coefficients with `diag`onal values of 1. Note that this matrix is ***not*** a `cor`relation matrix, because Tjøstheim's correlation coefficient
...
Function `spatstat.explore::fv()` creates a function value table object (`fv.object`), i.e., an R object of `S3` class `'fv'`. In addition to the existing `S3` method dispatches to the class `'fv'` in package **`spatstat.explore`**,

```{r}
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches `spatstat.explore::*.fv`'
library(spatstat.explore)
methods(class = 'fv') |> 
  attr(which = 'info', exact = TRUE) |>
  subset.data.frame(subset = from == 'spatstat.explore')
...
Package **`groupedHyperframe`** implements more `S3` method dispatches to the class `'fv'` (@tbl-S3_fv),

```{r}
#| code-fold: true
#| code-summary: '**Vignette**: `S3` method dispatches `groupedHyperframe::*.fv`'
#| label: tbl-S3_fv
...
S3_fv |>
  knitr::kable(
    caption = '`S3` method dispatches `groupedHyperframe::*.fv`'
  )
```
...
Function `spatstat.geom::anylist()` creates a `list` of objects of *any* type, i.e., an R object of `S3` class `'anylist'`.  

The `S3` class `'anylist'` `inherits` from the class `'listof'` defined in package **`stats`** shipped with vanilla `r R.version.string`.

In addition to the existing `S3` method dispatches to the class `'anylist'` in packages **`spatstat.geom`** and **`spatstat.explore`**,

```{r}
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches `spatstat.geom::*.anylist`'
library(spatstat.geom)
...
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches `spatstat.explore::*.anylist`'
library(spatstat.explore)
methods(class = 'anylist') |> 
  attr(which = 'info', exact = TRUE) |>
  subset.data.frame(subset = from == 'spatstat.explore')
...
Package **`groupedHyperframe`** implements more `S3` method dispatches to the class `'anylist'` (@tbl-S3_anylist),

```{r}
#| code-fold: true
#| code-summary: '**Vignette**: `S3` method dispatches `groupedHyperframe::*.anylist`'
#| label: tbl-S3_anylist
S3_ppp = methods(class = 'anylist') |> 
  attr(which = 'info', exact = TRUE) |>
  subset.data.frame(subset = from == 'groupedHyperframe') |>
  within.data.frame(expr = {
    generic = generic |> sprintf(fmt = '`%s`')
...
S3_ppp |> 
  knitr::kable(
    caption = '`S3` method dispatches `groupedHyperframe::*.anylist`'
  )
```

### Kernel Density of `numeric`-`vector`s {#sec-kerndens_anylist}

The `S3` method dispatch `kerndens.anylist()` finds the kernel `density`s of each `numeric`-`vector`.

```{r}
#| code-fold: true
#| code-summary: '**Example**: function `kerndens.anylist()`'
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: function `quantile.anylist()`, no `numeric`-`marks`'
spatstat.data::Kovesi$values |> 
  kerndens() |>
  is.null()
```

### Quantile of `numeric`-`vector`s {#sec-quantile_anylist}

The `S3` method dispatch `quantile.anylist()` finds the `quantile`s of each `numeric`-`vector`.

```{r}
#| code-fold: true
#| code-summary: '**Example**: function `quantile.anylist()`'
spatstat.geom::anylist(
  rnorm(1e2L),
  rnorm(1e2L)
) |> 
...
## `solist` From `anylist` {#sec-solist}

Function `spatstat.geom::solist()` returns an object of `S3` class `'solist'`, i.e., list of two-dimensional spatial objects. 

The `S3` class `'solist'` `inherits` from the class `'anylist'` (Appendix @sec-anylist).

```{r}
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches to `solist` in package **`spatstat.geom`**'
library(spatstat.geom)
...
## `ppplist` From `solist,anylist` {#sec-ppplist}
...
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches to `ppplist` in package **`spatstat.geom`**'
library(spatstat.geom)
methods(class = 'ppplist') |> 
  attr(which = 'info', exact = TRUE) |>
  subset.data.frame(subset = from == 'spatstat.geom')
```

Package **`groupedHyperframe`** implements the additional `S3` method dispatches in @tbl-S3_ppplist,

```{r}
#| code-fold: true
#| code-summary: 'Additional `S3` method dispatches to `ppplist`'
#| label: tbl-S3_ppplist
S3_ppp = methods(class = 'ppplist') |> 
  attr(which = 'info', exact = TRUE) |>
  subset.data.frame(subset = from == 'groupedHyperframe') |>
  within.data.frame(expr = {
    generic = generic |> sprintf(fmt = '`%s`')
...
S3_ppp |> 
  knitr::kable(
    caption = '`S3` method dispatches to class `\'ppplist\'`'
  )
```

### Kernel Density of `numeric`-`marks` {#sec-kerndens_ppplist}

The `S3` method dispatch `kerndens.ppplist()` finds the kernel `density`s of all `ppp.object`s (Appendix @sec-quantile_ppp). Note that this is not `spatstat.explore::density.ppplist()`!

```{r}
#| code-fold: true
#| code-summary: '**Example**: function `kerndens.ppplist()`'
spatstat.data::betacells |>
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: function `kerndens.ppplist()`, no `numeric`-`marks`'
spatstat.data::btb.extra |> 
  kerndens()
```

### Quantile of `numeric`-`marks` {#sec-quantile_ppplist}

The `S3` method dispatch `quantile.ppplist()` finds the `quantile`s of all `ppp.object`s (Appendix @sec-quantile_ppp).

```{r}
#| code-fold: true
#| code-summary: '**Example**: function `quantile.ppplist()`'
spatstat.data::betacells |>
  spatstat.geom::split.ppp(f = 'type') |>
  quantile()
```
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: function `quantile.ppplist()`, no `numeric`-`marks`'
spatstat.data::btb.extra |> 
  quantile()
```
...
### Split by $k$-Means Clustering {#sec-split_kmeans-ppplist}
...
## `vectorlist` From `anylist` {#sec-vectorlist}
...
### Validity {#sec-is_vectorlist}

Function `is.vectorlist()` inspects whether an `'anylist'` has all-`vector` elements of the same `length`, using the workhorse function `base::is.vector()`.

```{r}
#| code-fold: true
#| code-summary: '**Example**: function `is.vectorlist()`'
spatstat.data::Kovesi$values |>
  is.vectorlist()
spatstat.data::Kovesi$values |>
...
In @sec-summary_fv, the `numeric`-hypercolumns created by function `summary_fv()` are also `'vectorlist'`s (although not supported as hypercolumns in `hyperframe` as of package **`spatstat.geom`** v`r packageVersion('spatstat.geom')`), as they contain tabulated values on common $r$-`vector`(s). A "slice" of the hypercolumn *`out_fv$hladr.E.cumvtrapz`* at the 5th index of the $r$-vector, i.e., $r=50$, may be extracted by calling the `S3` method dispatch `spatstat.geom::with.hyperframe()`, but the `S3` method dispatch `t.vectorlist()` is much faster.

```{r}
#| code-fold: true
#| code-summary: "**Example**: a 'slice' from `hypercolumn` *`out_fv$hladr.E.cumvtrapz`*"
E_r50_a = out_fv |>
...
## `fvlist` From `anylist` {#sec-fvlist}
...
## `hyperframe` {#sec-hyperframe}

Function `spatstat.geom::hyperframe()` returns a `hyperframe`, i.e., an object of `S3` class `'hyperframe'`. Among
...
### Kernel Density {#sec-kerndens-hyperframe}

The `S3` method dispatch `kerndens.hyperframe()` finds the kernel `density`s

-   of the numeric-hypercolumns (Appendix @sec-kerndens_anylist), and
-   of the numeric marks in the one-and-only-one `ppp`-hypercolumn (Appendix @sec-isnumeric-ppp, @sec-kerndens_ppp, @sec-kerndens_ppplist).

and appends additional `numeric`-hypercolumns to the input. Example has been discussed extensively in @sec-summary_kerndens.

### Quantile {#sec-quantile-hyperframe}

The `S3` method dispatch `quantile.hyperframe()` finds the `quantile`s

-   of the numeric-hypercolumns (Appendix @sec-quantile_anylist), and
-   of the numeric marks in the one-and-only-one `ppp`-hypercolumn (Appendix @sec-isnumeric-ppp, @sec-quantile_ppp, @sec-quantile_ppplist).

and appends additional `numeric`-hypercolumns to the input. Example has been discussed extensively in @sec-summary_quantile.
...
The `S3` method dispatch `spatstat.geom::superimpose.ppplist()` superimposes *all* `ppp.object`s from *all* input `ppplist`s, as of package **`spatstat.geom`** v`r packageVersion('spatstat.geom')`, instead of performing a by-element superimpose of all input `ppplist`s as intended for the `S3` method dispatch `superimpose.hyperframe()`.

```{r}
#| code-fold: true
#| code-summary: '**Review**: `spatstat.geom::superimpose.ppplist()` *not* what we need'
#| warning: false
...
## `groupedHyperframe` {#sec-groupedHyperframe-appendix}

### Nested Groups {#sec-nestedGroup}
...
### Aggregation {#sec-aggregate_groupedHyperframe}

The `S3` method dispatch `aggregate.groupedHyperframe()`

-   removes the `ppp`- and `fv`-hypercolumns of the input `groupedHyperframe`;
-   aggregates the columns, i.e., *not* hypercolumns, of the input;
-   aggregates the `vectorlist`-hypercolumns (Appendix @sec-vectorlist) of the input;
-   returns a `hyperframe`.

The aggregation may be specified at either one of the nested grouping levels $g_1,\cdots,g_{m-1}$. Aggregation at the lowest grouping level $g_m$ is ignored, i.e., no aggregation to be performed. Available aggregation methods in parameter `fun` are the point-wise minima `base::pmin()`, maxima `base::pmax()`, means `pmean()` (default) and medians `pmedian()`.

Examples are provided in @sec-aggregate.
