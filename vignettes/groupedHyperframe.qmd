...
The derived class `'vectorlist'` is not part of package **`spatstat.geom`** as of dev v`r packageVersion('spatstat.geom')`.  Users may call the `S3` method dispatch `t.vectorlist()` explicitly. 

```{r}
#| code-fold: true
#| code-summary: 'Workaround before package **`spatstat.geom`** (ever) implements class `vectorlist`'
stopifnot(identical(Kovesi_values_t, t.vectorlist(spatstat.data::Kovesi$values)))
```
