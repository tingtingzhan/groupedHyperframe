...
-   the additional predictors and/or endpoints for downstream analysis, e.g., *`OS`*, *`gender`* and *`age`*, before the `|` separator on the right-hand-side. Alternatively, user may use the dot symbol `.` to denote the remaining variables except for those mentioned in the `marks` and the nested grouping structure. The grammar of dot symbol `.` is explained in section *Details* of [`stats::formula`](https://search.r-project.org/R/refmans/stats/html/formula.html) documentation, as well as section *Arguments* of [`stats::terms.formula`](https://search.r-project.org/R/refmans/stats/html/terms.formula.html) documentation.
...
The `S3` method dispatch `t.vectorlist()` transposes a `'vectorlist'` of equi-`length`.

```{r}
Kovesi_values_t = Kovesi_values |> t()
```

The `S3` method dispatch `t.vectorlist()` returns a `'vectorlist'`, but with the `length` and `lengths` of the input switched.  The use of the term "transpose", and the extension of the generic function `base::t()`, mirrors the use of `base:::t.default()` to transpose a `'matrix'`, i.e., to switch the `ncol` and `nrow` of the input `'matrix'`.   The returned object *`Kovesi_values_t`* is a `length`-256 `listof` `vector`s, all elements of which have `length`-41.
...
| [`anylist`](https://search.r-project.org/CRAN/refmans/spatstat.geom/html/anylist.html) | List of objects, from package **`spatstat.geom`** |
