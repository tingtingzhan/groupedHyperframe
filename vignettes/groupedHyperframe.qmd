...
Note that we intentionally make the toy example data *`x`* *not* equidistant, as neither the chained trapezoidal rule framework nor the function `pracma::trapz()` *requires* the equidistant assumption.
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: `is.numeric.ppp()` on `'none'`-`markformat`"
spatstat.data::cells
spatstat.data::cells |>
  is.numeric()
```
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: `is.numeric.ppp()` on `'vector'`-`markformat`"
...
```{r}
#| code-fold: true
#| code-summary: "**Review** & **Example**: `is.numeric.ppp()` vs. `spatstat.geom::is.multitype.ppp()` on `'dataframe'`-`markformat`"
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: `na.omit.ppp()` and  `na.exclude.ppp()` on `'none'`-`markformat`"
spatstat.data::cells
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: `na.omit.ppp()` and  `na.exclude.ppp()` on `'vector'`-`markformat`, no missingness"
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: `na.omit.ppp()` and  `na.exclude.ppp()` on `'dataframe'`-`markformat`"
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: `grouped_ppp()` with one `mark`"
s_a = wrobel_lung |>
...
The `S3` generic function `split_kmeans()` splits `ppp.object`, `ppplist` (Appendix @sec-ppplist), and `hyperframe` (Appendix @sec-split_kmeans-hyperframe) by $k$-means clustering. We use a `subset` of the data set *`flu`* from package **`spatstat.data`** to illustrate the various `S3` method dispatches of `split_kmeans()`.
...
```{r}
#| code-fold: true
#| code-summary: "**Review**: `spatstat.explore::fvnames(., a = '.y')`"
spatstat.data::spruces |> 
  spatstat.explore::markcorr() |>
  spatstat.explore::fvnames(a = '.y')
```

Function `keyval.fv()` finds various function values (default being the black-solid-curve) in an `fv.object`, with the corresponding function argument `spatstat.explore::fvnames(., a = '.x')` as the `vector` `names`. Function `keyval.fv()` is a "psuedo" `S3` method dispatch, because we do not have an `S3` generic function `keyval()` defined as of package **`groupedHyperframe`** v`r packageVersion('groupedHyperframe')`.

```{r}
#| code-fold: true
#| code-summary: '**Example**: `keyval.fv()`'
spatstat.data::spruces |> 
  spatstat.explore::markcorr() |>
  keyval.fv() |>
  head(n = 3L)
...
Function `spatstat.explore::markcorr()` provides a default argument of parameter $r$ (Appendix @sec-default_rmax), at which the mark correlation function $k_f(r)$ are evaluated. The `S3` method dispatch `spatstat.explore::print.fv()` prints the *recommended range* and *available range* of $r$.
...
Function `lastLegal()` provides the *index* of the last *consecutive* legal value in a `double` `vector`, i.e., the first exceptional/illegal value of `0`, `NaN`, or `Inf` appears at the next index. "Legal" in function `lastLegal()` is defined as a `double` scalar being not-`NaN`, not-`Inf`, and with `abs`olute value greater than `.Machine$double.eps`.
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: `lastLegal()`, toy examples'
c(exp(1), pi) |> 
  lastLegal()
c(exp(1), pi, NaN) |> 
  lastLegal()
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: `lastLegal()` of `keyval.fv()`'
spatstat.data::spruces |> 
  spatstat.explore::markcorr(r = 0:100) |>
  keyval.fv() |>
  lastLegal()
...
Legality of the function `spatstat.explore::markcorr()` returns depends not only on the input `ppp.object`, but also on the values of $r$. In other words, the creation of an `fv.object` by function `markcorr()` is a numerical procedure. Therefore, the discussion of Legal $r_\text{max}$ pertains to `fv.object` (Appendix @sec-fv), instead of to `ppp.object` (Appendix @sec-ppp).
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: `rmax_.ppplist()`'
...
The `S3` generic function `split_kmeans()` has been introduced in Appendix @sec-split_kmeans. The `S3` method dispatch `split_kmeans.listof()` splits a `ppplist` by $k$-means clustering. We use a `subset` of the data set *`flu`* from package **`spatstat.data`** to illustrate this concept.  The returned object has `attributes`
...
The batch process `Gcross_()` (@sec-batch) adds an `fv`-hypercolumn *`m.G`* to the input `hyperframe` *`fluM`*.  The `message` of "Legal $r_\text{max}$" provides the indices in the `fv`-hypercolumn, i.e., the row indices of the `hyperframe`, where the legal $r_\text{max}$ (Appendix @sec-legal_rmax) is smaller than the user-specified $r_\text{max}$.
 
```{r}
#| code-fold: true
#| code-summary: '**Example**: `Gcross_()`'
...
Function `as.fvlist()` inspects a `listof` `fv.object`s and convert it to an object of class `'fvlist'`, before `spatstat.geom::hyperframe()` (ever) recognizes the class `'fvlist'`.  To be converted to an `fvlist`, the `listof` `fv.object`s must have `identical`

-   names of function argument, i.e., returns of `spatstat.explore::fvnames(., a = '.x')` per `fv.object`;
-   function arguments, i.e., the values of $r$ per `fv.object`;
-   names of recommended function value, i.e., returns of `spatstat.explore::fvnames(., a = '.y')` per `fv.object`;
-   function names, i.e., returns of `attr(., 'fname')` per `fv.object`.


```{r}
#| code-fold: true
#| code-summary: '**Example**: `as.fvlist()`'
m_G = fluM_G$m.G |>
  as.fvlist()
m_G |> 
  class()
...
-   `attr(., 'r')`, a `numeric` `vector`, the common values of `r` shared by all `fv.object`s;
-   `attr(., '.y')`, a `character` scalar, the name of the recommended function value, i.e., the common returned value of `spatstat.explore::fvnames(., a = '.y')` from all `fv.object`s;
-   `attr(., 'rmax')`, a `numeric` scalar, the minimum Legal $r_\text{max}$ (Appendix @sec-legal_rmax) of all `fv.object`s.


```{r}
#| code-fold: true
#| code-summary: "**Example**: `base::attributes()` of class `'fvlist'`"
m_G |> 
  attributes()
```


The `S3` method dispatch `summary.fvlist()` performs truncated summarization at a user-specified $r_\text{max}$. The `S3` method dispatch `summary.fvlist()` returns a `list` with elements 

-   *`$y`*, the *function values*, i.e., the black-solid-curve (Appendix @sec-blackSolidCurve) per `fv.object`;
-   *`$cumtrapz`*, the *cumulative trapezoidal integration* (Appendix @sec-trapz) under the black-solid-curve per `fv.object`;
-   *`$cumvtrapz`*, the *cumulative average vertical height of trapezoidal integration* (Appendix @sec-vtrapz) under the black-solid-curve per `fv.object`.


```{r}
#| code-fold: true
#| code-summary: '**Example**: `summary.fvlist()`, truncated at $r_\text{max}>$ minimum Legal $r_\text{max}$ of all `fv.object`s'
m_G_s1 = m_G |>
  summary(rmax = 85, data.name = 'm.G')
m_G_s1 |> 
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: `summary.fvlist()`, truncated at $r_\text{max}\leq$ minimum Legal $r_\text{max}$ of all `fv.object`s'
m_G_s2 = m_G |>
  summary(rmax = 70, data.name = 'm.G')
m_G_s2 |> 
  lapply(FUN = head, n = 2L)
```
...
Several of them behave *very differently* from their counterpart in vanilla `r R.version.string`. Users are strongly advised to read the documentation of these functions from package **`spatstat.geom`**.
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: `get_nested()`, exception handling'
(~a) |> get_nested()
(~b:c) |> get_nested()
```
