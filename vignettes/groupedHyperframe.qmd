...
The `message` of "Legal $r_\text{max}$" provides the indices in the `ppp`-hypercolumn, i.e., the row indices of the `hyperframe`, where the legal $r_\text{max}$ (see @sec-legal_rmax) is smalller than the user-specified $r_\text{max}$. The returned `hyperframe` (and/or `groupedHyperframe`) has
...
Interaction terms using colon operator (`:`) are allowed in the grouping structure, such as `m1+m2 ~ x1 + x2 | g1/g2a:g2b/g3a:g3b:g3c`. This feature is made possible because in R `language` (especially `formula`) object, the colon operator (`:`) has higher priority than the forward slash (`/`).
...
The helper function `get_nested()` breaks down the nested grouping structure by the forward slash (`/`).
