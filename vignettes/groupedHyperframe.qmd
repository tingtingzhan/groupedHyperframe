...
(A) our toy example; 
(B) kernel `density` object
(C) returned object of function `spatstat.explore::Emark()`.  [Say some English]{style="background-color: #FFFF00"}
(D) returned object of function `spatstat.explore::Vmark()`.  [Say some English]{style="background-color: #FFFF00"}

```{r}
#| code-fold: true
#| code-summary: "R code: visualize `vtrapz()` and `cumvtrapz()`, panel (A)"  
...
```{r}
#| code-fold: true
#| code-summary: "R code: visualize `vtrapz()` and `cumvtrapz()`, panel (B)"  
fig_vtrapz_2 = rnorm(n = 1e3L) |> density() |> visualize_vtrapz() + 
...
```{r}
#| code-fold: true
#| code-summary: "R code: visualize `cumvtrapz()`, panel (C)"  
fig_vtrapz_3 = spatstat.data::spruces |>
  spatstat.explore::Emark() |>
  visualize_vtrapz(draw.v = FALSE, draw.rect = FALSE) +
  ggplot2::labs(title = '(C). spatstat.explore::Emark()') + 
  ggplot2::theme_minimal()
```

```{r}
#| code-fold: true
#| code-summary: "R code: visualize `cumvtrapz()`, panel (D)"  
fig_vtrapz_4 = spatstat.data::spruces |>
  spatstat.explore::Vmark() |>
  visualize_vtrapz(draw.v = FALSE, draw.rect = FALSE) +
  ggplot2::labs(title = '(D). spatstat.explore::Vmark()') +
  ggplot2::theme_minimal()
```
...
library(patchwork)
fig_vtrapz_1 + fig_vtrapz_2 + fig_vtrapz_3 + fig_vtrapz_4
```
