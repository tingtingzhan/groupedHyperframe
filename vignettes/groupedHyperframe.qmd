...
-   `r knitr::asis_output('\U1f5dd')` **`spatstat.explore`** (developer version `r packageVersion('spatstat.explore')`) and **`spatstat.geom`** (developer version `r packageVersion('spatstat.geom')`), key dependency, for spatial statistics

Package **`groupedHyperframe`** `Suggests` packages

-   `r knitr::asis_output('\U1f5dd')` **`spatstat.data`** (developer version `r packageVersion('spatstat.data')`), key suggest, for example spatial data set
-   `r knitr::asis_output('\U1f5dd')` **`survival`** [@survival, version `r packageVersion('survival')`], key suggest, for function `survival:::as.data.frame.Surv()` to help `hyperframe` understand `Surv` object
...
The S3 method dispatch `superimpose.hyperframe()`, which does not exist in package **`spatstat.geom`** as of version `r packageVersion('spatstat.geom')`, superimposes multiple `hyperframe`s if-and-only-if they have identical 
...
-   grouping structure, i.e., `attr(., 'group')`
...
```{r}
#| code-fold: true
#| code-summary: Number of points per `ppp.object` in *`fluM`*
fluM$pattern |>
...
The `S3` method dispatch `superimpose.hyperframe()` combines `hyperframe`s *`fluM2`* and *`fluM1`*; the order matters!  
...
```{r}
#| code-fold: true
#| code-summary: Not here yet; author is consulting Dr. Baddeley
identical(fluS, fluM)
```

The returned `hyperframe` *`fluS`* has `identical` `ppp`-hypercolumn as *`fluM`*.

```{r}
stopifnot(identical(fluS$pattern, fluM$pattern))
```
