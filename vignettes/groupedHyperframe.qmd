...
### of `numeric` information {#sec-summary_quantile}
...
-   of the numeric marks in the one-and-only-one `ppp`-hypercolumn (Appendix @sec-isnumeric-ppp, @sec-quantile_ppp and @sec-quantile_ppplist).

and appends additional `numeric`-hypercolumns to the input.  In the following example, the returned object has

-   numeric-hypercolumn *`phenotype.nncross.quantile`*, the `quantile`s of numeric-hypercolumn *`phenotype.nncross`*.
-   numeric-hypercolumn *`hladr.quantile`*, the `quantile`s of the numeric mark *`hladr`* in `ppp`-hypercolumn.
...
```{r}
#| code-fold: true
#| code-summary: 'A `groupedHyperframe` object *`out_q`*: summarizing `quantiles`'
out_q
```
...
The `S3` method dispatch `spatstat.geom::superimpose.ppplist()` superimposes *all* `ppp.object`s from *all* input `ppplist`s, as of package **`spatstat.geom`** v`r packageVersion('spatstat.geom')`, instead of performing a by-element superimpose of all input `ppplist`s as intended for the `S3` method dispatch `spatstat.geom::superimpose.hyperframe()`.
