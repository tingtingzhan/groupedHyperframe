...
We present an `S3` generic function `rmax_()` to obtain the *Recommended* $r_\text{max}$. The primary `S3` method dispatch `rmax_.ppp()` is an off-label use of the un-documented functions `spatstat.explore::rmax.rule()` and `spatstat.geom::handle.r.b.args()`. Together with the additional `S3` method dispatches `rmax_.ppplist()` and `rmax_.hyperframe()`, user may obtain the *Recommended* $r_\text{max}$ *before* applying the (potentially) very slow batch processes.
