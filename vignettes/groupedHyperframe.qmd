...
🚧 A paragraph about functions `key1.fv()` and `key1val.fv()`, based on function `spatstat.explore::plot.fv(., do.plot = FALSE)`.
...
Functions `trapz.fv()` and `cumtrapz.fv()` provide the (cumulative) trapezoidal integration under the black-solid curve.  🚧 Expand this paragraph.
...
| CRAN, R, `Depends`, `Imports`, `Enhances` | The Comprehensive R Archive Network, <https://cran.r-project.org>.  [*Writing R Extensions*, Section 1.1.3 *Package Dependencies*](https://cran.r-project.org/doc/manuals/r-devel/R-exts.html#Package-Dependencies) |
