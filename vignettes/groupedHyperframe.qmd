...
🚧 [Add a paragraph about truncation at Legal $r_\text{max}$]{style="background-color: yello"}
...
🚧 [Add a paragraph about truncation at user-specified $r_\text{max}$]{style="background-color: yello"}
...
As we have mentioned in the previous section *Batch processes*, a same user-specified argument of `r` will be used for *all* `ppp.object`s in the `ppp`-hypercolumn. Suppose a naive user uses an $r$-vector well beyond the *recommended range* and/or *available range*. In this case, function `aggregate_fv()` prints a `message` of [*Legal* $r_\text{max}$, i.e., the largest $r$-values, that no value of `0`, `NaN` and/or `Inf` appears in *each* of the `fv`-returns]{style="background-color: #FFFF00"}, e.g., in the hypercolumn *`hladr.E`*.
...
We may observe exceptional returns if we go beyond the *recommended range* and/or *available range*. In the following example, we see that the mark correlation $k_f(r)$ (column *`iso`*) having values `0`, `NaN` and `Inf`.
