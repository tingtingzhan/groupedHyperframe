...
The `S3` method dispatch `is.numeric.ppp()` determines if a `ppp.object` contains `numeric` `marks`.
...
```{r}
#| code-fold: true
#| code-summary: "**Example**: `is.numeric.ppp()` on `'dataframe'`-`markformat`"
...
```{r}
#| code-fold: true
#| code-summary: "**Review**: `spatstat.geom::is.multitype.ppp()` on `'dataframe'`-`markformat`"
spatstat.data::betacells
spatstat.data::betacells |>
  is.numeric()
spatstat.data::betacells |>
