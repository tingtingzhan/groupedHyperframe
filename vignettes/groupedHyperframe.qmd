...
as well as @Yi25; @Yi23a; @Yi23b, was featured with a non-spatial `hyperframe` *`Ki67q`* with a numeric-hypercolumn *`logKi67.quantile`*. Functions in the R code-chunk below are explained in @sec-groupedHyperframe.
...
#| message: false
#| code-fold: true
#| code-summary: '`S3` method dispatches to `ppp.object` in package **`spatstat.geom`**'
library(spatstat.geom)
methods(class = 'ppp') |> 
...
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches to `ppp.object` in package **`spatstat.explore`**'
...
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches to `fv.object` in package **`spatstat.explore`**'
library(spatstat.explore)
methods(class = 'fv') |> 
  attr(which = 'info', exact = TRUE) |>
  subset.data.frame(subset = from == 'spatstat.explore')
...
#| message: false
#| code-fold: true
#| code-summary: 'Existing `S3` method dispatches to `anylist` in package **`spatstat.geom`**'
library(spatstat.geom)
...
Package **`groupedHyperframe`** implements the additional `S3` method dispatches in @tbl-S3_anylist,

```{r}
#| code-fold: true
#| code-summary: 'Additional `S3` method dispatches to `anylist`'
...
S3_ppp |> 
  knitr::kable(
    caption = '`S3` method dispatches to class `\'anylist\'`'
  )
```
...
The authors use the hypercolumn *`s$ppp.`* (@sec-grouped_ppp) to illustrate this concept.
...
The `S3` method dispatch `quantile.ppplist()` finds the `quantile`s of all `ppp.object`s (Appendix @sec-quantile_ppp).  The authors use the hypercolumn *`s$ppp.`* (@sec-grouped_ppp) to illustrate this concept.
