...
Many function in package **`spatstat.explore`** calls the workhorse function `spatstat.explore::fv()` and returns an `fv.object`, i.e., an object with `S3` class `'fv'`.  In addition to these

```{r}
#| message: false
#| code-fold: true
#| code-summary: '`S3` method dispatches to `fv.object` defined in package **`spatstat.geom`**'
