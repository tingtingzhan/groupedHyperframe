...
-   There is not an S3 method dispatch `merge.hyperframe` in package **`spatstat.geom`**;
-   Should the authors decide to implement the `merge` functionality for `hyperframe` class, at least two S4 method dispatches need to be written
    -   `setMethod(f = merge, signature = c(x = 'hyperframe', y = 'data.frame'), ...)`
    -   `setMethod(f = merge, signature = c(x = 'hyperframe', y = 'hyperframe'), ...)`
-   Should the authors decide to implement the `merge` functionality for `groupedHyperframe` class, at least three S4 method dispatches need to be written
    -   `setMethod(f = merge, signature = c(x = 'groupedHyperframe', y = 'data.frame'), ...)`
    -   `setMethod(f = merge, signature = c(x = 'groupedHyperframe', y = 'hyperframe'), ...)`
    -   `setMethod(f = merge, signature = c(x = 'groupedHyperframe', y = 'groupedHyperframe'), ...)`, for this the authors need to consider the (potentially different) grouping structures of `x` and `y` inputs.
