...
Stable releases to [`CRAN`](https://CRAN.R-project.org/package=groupedHyperframe) are typically updated every 2 to 3 months, or when the authors have an upcoming peer-reviewed manuscript in the review process. [Developers should **not** use the `CRAN` version!]{style="background-color: #FFFF00"}
...
```{r}
m = spatstat.data::spruces |> 
  spatstat.explore::markcorr()
m
```

We may observe exceptional returns if we go beyond the *recommended range* and/or *available range*. In the following example, we see the mark correlation value $k_f(r)$ of `NaN` at $r=81,88,89,90$, `0` at $r=82$ and `Inf` at $r=83,84,87$.
