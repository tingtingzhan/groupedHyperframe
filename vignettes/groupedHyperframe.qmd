...
```{r}
#| code-fold: true
#| code-summary: 'Check the content of `spatstat.geom::as.list.hyperframe() |> base::list2env()`'
ls(envir = ev) |>
  mget(envir = ev) |>
  lapply(FUN = class)
```
...
The authors suggest users use the pipeline of functions `base::merge.data.frame()` and `as.groupedHyperframe()` as a workaround.
