...
c(
  ...
  '`trapz`, `cumtrapz`, `cumarea`', '(Cumulative) trapezoidal integration', '`pracma::trapz`; `pracma::cumtrapz`'
) |>
...
Also, end user should remove parameter `mc.cores = 1L` in all examples and use the default option, which makes use of all CPU cores on the current host for macOS. We are forced to have `mc.cores = 1L` in this vignette just to pass CRAN's submission requirement.
