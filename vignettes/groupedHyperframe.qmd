...
We define the *average height* of a trapezoidal integration as equation (@eq-trapz) divided by $N\Delta x$, the length of $x$-domain,
...
In R nomenclature, the term "cumulative" indicates that *the `i`th element of the output is the operation result of the first-`i`-elements of the input*.  Function `pracma::cumtrapz()` calculates a cumulative trapezoidal integration. Note that the first element of the output is always `0`, as a trapezoidal integration needs at least the first and second elements $x_0$ and $x_1$. 

```{r}
pracma::cumtrapz(x, y)
```

Function `cumvtrapz()` calculates the cumulative average height of a trapezoidal integration.  Note that the first element of the output is always `NaN`, which is a result of `0` (trapzoidal integration) divided by `0` (length of $x$-domain).
