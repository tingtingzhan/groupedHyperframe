...
-   **`spatstat.explore`** (developer version `r packageVersion('spatstat.explore')`), with
    -   new methods for `fv.object`
    -   batch processes to return a hypercolumn of `fv.object`s
-   **`spatstat.geom`** (developer version `r packageVersion('spatstat.geom')`), with
    -   a new derived class of `hyperframe`
    -   new methods for `ppp.object`
...
An `S3` class `hyperframe`, for **hyper data frame**, is defined in package **`spatstat.geom`** [@spatstat15; @spatstat05]. A `hyperframe` contains columns either as `vector`s like in a `data.frame`, or as `list`s of objects of the same class, a.k.a., the `hypercolumns`. The `hyperframe` data structure is particularly useful in spatial analysis, e.g., with medical images, where the spatial information in each image would be represented by one element in a hypercolumn. For example, example data ***`demohyper`*** from package **`spatstat.data`** contains a `factor` column *`Group`*, a point-pattern `ppp`-hypercolumn *`Points`*, and a pixel image `im`-hypercolumn *`Image`*. Several `S3` method dispatches to the `hyperframe` class behave *very differently* from their counterpart in vanilla R, see Appendix @sec-hyperframe.
...
-   the *function values*, i.e., the black-solid-curve (see Appendix @sec-blackSolidCurve) determined by function `spatstat.explore::plot.fv()`.
-   the *cumulative trapezoidal integration* (see Appendix @sec-blackSolidCurve) under the black-solid-curve.
...
| [`base::within`](https://search.r-project.org/R/refmans/base/html/with.html) | [`base::within.data.frame`](https://search.r-project.org/R/refmans/base/html/with.html) | do not exist (as of version `r packageVersion('spatstat.geom')`) |

: `S3` methods `*.hyperframe()` that are drastically different from `*.data.frame()` or `*.default()`

### Superimpose multiple `hyperframe`s

The S3 method dispatch `superimpose.hyperframe()`, which does not exist in package **`spatstat.geom`** as of version `r packageVersion('spatstat.geom')`, `superimpose`s multiple `hyperframe`s if-and-only-if they have identical 

-   `dim`ensions, via function `spatstat.geom::dim.hyperframe()`
-   columns, i.e., `unclass(.)$df`
-   `names` and `class` of the `hypercolumn`s, i.e., `unclass(.)$hypercolumns`

We use a subset of the data set `flu` from package **`spatstat.data`** as a toy example.
...
```{r}
#| code-fold: true
#| code-summary: Subset 'M2-M1' of data set spatstat.data::flu
fluM
```

In this subset of `r nrow(fluM)` subjects, each of them has a `ppp.object` with 200-500 points.
...
We create a `hyperframe` *`fluM1`* which consists of the *`M1`* marks only, and another `hyperframe` *`fluM2`* which consists of the *`M2`* marks only.

```{r}
#| code-fold: true
#| code-summary: Some tedious code to create `fluM1` and `fluM2`
# authors are consulting Dr. Baddeley for potential update to \CRANpkg{spatstat.geom}
fluM1 = fluM2 = unclass(fluM)
fluM1$hypercolumns$pattern = fluM1$hypercolumns$pattern |> 
  lapply(FUN = \(i) {
    spatstat.geom::subset.ppp(i, subset = (marks == 'M1'), drop = TRUE)
  })
class(fluM1$hypercolumns$pattern) = unclass(fluM)$hypercolumns$pattern |> class()
fluM2$hypercolumns$pattern = fluM2$hypercolumns$pattern |> 
  lapply(FUN = \(i) {
    spatstat.geom::subset.ppp(i, subset = (marks == 'M2'), drop = TRUE)
  })
class(fluM2$hypercolumns$pattern) = unclass(fluM)$hypercolumns$pattern |> class()
class(fluM1) = class(fluM2) = class(fluM)
```
...
```{r}
#| code-fold: true
#| code-summary: Number of points per `ppp.object` in *`fluM1`*
fluM1$pattern |>
  sapply(FUN = spatstat.geom::npoints)
```
...
```{r}
#| code-fold: true
#| code-summary: Number of points per `ppp.object` in *`fluM2`*
fluM2$pattern |>
  sapply(FUN = spatstat.geom::npoints)
```

The `S3` method dispatch `superimpose.hyperframe()` combines `hyperframe`s *`fluM2`* and *`fluM1`*; the order matters!  The returned object is `identical` to *`fluM`*.
...
Function `key1val.fv()` finds the function values in an `fv.object`, with the corresponding $r$-values as the `vector` `names`. Function `key1val.fv()` is also a "psuedo" `S3` method dispatch, because we do not have an `S3` generic function `key1val()` defined (as of package **`groupedHyperframe`** version `r packageVersion('groupedHyperframe')`).
...
Functions `trapz.fv()` and `cumtrapz.fv()` calculate the (cumulative) trapezoidal integration under the black-solid-curve, with the help of functions `pracma::trapz()` and `pracma::cumtrapz()`. Both functions `trapz.fv()` and `cumtrapz.fv()` are "psuedo" `S3` method dispatches, as the workhorse functions in package **`pracma`** (as of version `r packageVersion('pracma')`) are not `S3` generic functions.
...
| `Depends`, `Imports`, `Enhances` | [*Writing R Extensions*, Section 1.1.3 *Package Dependencies*](https://cran.r-project.org/doc/manuals/r-devel/R-exts.html#Package-Dependencies) |
...
| [`marks`](https://search.r-project.org/CRAN/refmans/spatstat.geom/html/spatstat.geom-internal.html), [`is.marked`](https://search.r-project.org/CRAN/refmans/spatstat.geom/html/is.marked.html) | Marks of a point pattern |
