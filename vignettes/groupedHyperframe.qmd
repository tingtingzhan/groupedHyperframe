...
<details><summary>`library(survival)` in @sec-start</summary>
To put the un-exported `S3` method dispatch `survival:::as.data.frame.Surv()` on the `search` path. This is necessary, so that the function `spatstat.geom::hyperframe()` could treat a `Surv` object as a column instead of a `hypercolumn`.
</details>

<details><summary>`library(patchwork)` in Appendix @sec-vtrapz</summary>
...
<details><summary>`library(spatstat.geom)` in Appendix @sec-ppp and @sec-hyperframe, and `library(spatstat.explore)` in Appendix @sec-fv</summary>  
...
<details><summary>explicitly without namespace, if they are exported from package **`base`**, **`stats`**, **`utils`**, etc.</summary>  
...
<details><summary>by their generic functions without namespace, if they are un-exported from package **`base`**, **`stats`**, **`utils`**, etc.</summary>  
...
<details><summary>explicitly with namespace, no matter they are exported or un-exported from [`contrib`uted packages](https://cran.r-project.org/web/packages)</summary>  
...
<details><summary>in package **`groupedHyperframe`** by their generic functions without namespace</summary>  

e.g., the authors use `library(groupedHyperframe); log()` instead of 

-   `library(groupedHyperframe); log.ppp()` or 
-   `groupedHyperframe::log.ppp()`.

All `S3` method dispatches in package **`groupedHyperframe`** are exported.
</details>


Data sets are always called with its namespace. 

<details><summary>data from package **`datasets`** shipped with vanilla `r R.version.string`</summary>  
...
<details><summary>data from [`contrib`uted packages](https://cran.r-project.org/web/packages)</summary>  
...
set.seed(12); append_marks(ves) = rlnorm(n = spatstat.geom::npoints(ves))
ves |> spatstat.geom::print.ppp()
```
...
set.seed(23); append_marks(wak) = rlnorm(n = spatstat.geom::npoints(wak))
wak |> spatstat.geom::print.ppp()
```
...
set.seed(42); append_marks(urki) = rlnorm(n = spatstat.geom::npoints(urki))
urki |> spatstat.geom::print.ppp()
```
...
set.seed(33); append_marks(stonest) = rlnorm(n = spatstat.geom::npoints(stonest))
stonest |> spatstat.geom::print.ppp()
```
...
Functions `trapz.fv()` and `cumtrapz.fv()` calculate the (cumulative) trapezoidal integration (Appendix @sec-trapz) under the black-solid-curve, using the $r$-values as `x` and the `key1val.fv()` as `y`. They are both "psuedo" `S3` method dispatches, as the workhorse functions `pracma::trapz()` and `pracma::cumtrapz()` are not `S3` generic functions as of package **`pracma`** [@pracma, v`r packageVersion('pracma')`].
...
```{r}
#| code-fold: true
#| code-summary: '**Example**: `lastLegal()` of `key1val.fv()`'
spatstat.data::spruces |> 
  spatstat.explore::markcorr(r = 0:100) |>
  key1val.fv() |>
  lastLegal()
