...
Exceptional/illegal values `0`, `NaN` and `Inf` may appear in the mark correlation function $k_f(r)$, if the $r$-`vector` goes beyond the recommended and/or available range.
...
Function `lastLegal()` returns the *index* of the last *consecutive* legal value in a `double` `vector`, i.e., the first exceptional/illegal value of `0`, `NaN`, or `Inf` appears at the next index. "Legal" in function `lastLegal()` is defined as a `double` scalar being not-`NaN`, not-`Inf`, and with `abs`olute value greater than `.Machine$double.eps`.
...
-   function names, i.e., returns of `attr(.,'fname')`.
...
-   `attr(.,'.y')`, a `character` scalar, the name of the recommended function value, i.e., the `identical` returned values of `spatstat.explore::fvnames(., a = '.y')` from all `fv.object`s;
...
-   *`$cumtrapz`*, the (truncated) cumulative trapezoidal integration (Appendix @sec-trapz) under the black-solid-curve per `fv.object`;
-   *`$cumvtrapz`*, the (truncated) cumulative average vertical height of trapezoidal integration (Appendix @sec-vtrapz) under the black-solid-curve per `fv.object`.
