...
-   `r '\U1f5dd'` **`GET`** [@GET, v`r packageVersion('GET')`],
key dependency, for global envelope test
...
fluM1$hypercolumns$pattern[] = fluM1$hypercolumns$pattern |> 
  lapply(FUN = \(i) {
    spatstat.geom::subset.ppp(i, subset = (marks == 'M1'))
  })
fluM2$hypercolumns$pattern[] = fluM2$hypercolumns$pattern |> 
  lapply(FUN = \(i) {
    spatstat.geom::subset.ppp(i, subset = (marks == 'M2'))
  })
...
```{r}
#| code-fold: true
#| code-summary: Number of *`M1`* points per `ppp.object` in hypercolumn *`pattern`* of hyperframe *`fluM1`*
fluM1$pattern |>
  sapply(FUN = spatstat.geom::npoints)
```
...
```{r}
#| code-fold: true
#| code-summary: Number of *`M2`* points per `ppp.object` in hypercolumn *`pattern`* of hyperframe *`fluM2`*
fluM2$pattern |>
  sapply(FUN = spatstat.geom::npoints)
```

User may recreate the hyperframe *`fluM`* by combining `hyperframe`s *`fluM2`* and *`fluM1`* using the `S3` method dispatch `superimpose.hyperframe()`.  Note that the order of *`fluM2`*-then-*`fluM1`* matters, because the points are arranged in *`M2`*-then-*`M1`* in the original hypercolumn *`fluM$pattern`*.
...
The `S3` generic `.kmeans()` performs $k$-means clustering. We name this generic function with a prefix `.` because the workhorse function `stats::kmeans()` is not an `S3` generic.  Here we use data set *`shapley`* from package **`spatstat.data`** to illustrate the use of the `S3` method dispatch `.kmeans.ppp()`.
...
-   `formula`, $x$- and/or $y$- coordinate(s) and/or numeric `marks`
...
The `S3` method dispatch `.kmeans.ppp()` returns an object of class `'kmeans'` as defined in R package **`stats`**.
...
#| results: hide
spatstat.data::shapley |> 
  .kmeans(formula = ~ x + y, clusterSize = 1e3L)
```
...
The `S3` generic function `split_kmeans()` splits `ppp.object`, `listof` `ppp.object`s, and `hyperframe` by $k$-means clustering.
...
The `S3` method dispatch `split_kmeans.listof()` splits a `listof` `ppp.object`s by $k$-means clustering.  The returned object has `attributes`

-   `attr(.,'.id')`, indices of the `ppp.object`s before splitting.
-   `attr(.,'.cluster')`, indices of $k$-means clusters, nested in `.id`.

```{r}
#| label: splitkm_listof
#| results: hide
flu$pattern[1:2] |> 
  split_kmeans(formula = ~ x + y, centers = 3L) 
```
...
| [`split`](https://search.r-project.org/R/refmans/base/html/split.html) | Divide into Groups and Reassemble | 
