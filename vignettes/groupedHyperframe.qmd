...
c(
  ...
  '[`listof`](https://search.r-project.org/R/refmans/stats/html/listof.html)', 'Lists of objects',
  '[`markformat`](https://search.r-project.org/CRAN/refmans/spatstat.geom/html/spatstat.geom-internal.html)', 'Storage mode of `marks`',
  '[`marks`](https://search.r-project.org/CRAN/refmans/spatstat.geom/html/spatstat.geom-internal.html)', 'Marks of a point pattern',
...
The `S3` method dispatch `as.groupedHyperframe.data.frame()`

1.  inspects the input `data.frame` by the (nested) grouping structure;
2.  identifies the column(s) with ***non-identical elements*** within the lowest group, and converts them into hypercolumns;
3.  returns a `groupedHyperframe`.

Data set **`Ki67.`** in this package has non-identical column *`logKi67`* in the nested grouping structure *`~ patientID/tissueID`*.

```{r}
(Ki67g = Ki67. |> as.groupedHyperframe(group = ~ patientID/tissueID))
```

Converting a `data.frame` with cell intensities, etc., into a `groupedHyperframe` reduces memory allocation, but does not reduce much the `save`d `file.size` if `xz` compression is used.
...
```{r}
(s = grouped_ppp(formula = hladr + phenotype ~ OS + gender + age | patient_id/image_id, data = wrobel_lung))
```

# Batch Process on `ppp`-hypercolumn

This section outlines the batch processes of spatial point pattern analyses applicable to the *one-and-only-one* `ppp`-hypercolumn of a `hyperframe`. These batch processes are not intended for a `hyperframe` with multiple `ppp`-hypercolumn in the foreseeable future, as that would require checking for name clashes in the `marks` from multiple `ppp`-hypercolumn.

## To return an `fv`-hypercolumns
...
## To return a numeric-hypercolumns
...
## Aggregation of numeric-hypercolumn and numeric marks in `ppp`-hypercolumn
...
The `S3` method dispatch `.kmeans.ppp()` performs $k$-means clustering based on one-or-more user-specified coordinates and/or `marks`.
