...
Below is a list of terms used in this vignette.
...
c(
  ...
  '`fv`', 'Function value table', '`?spatstat.explore::fv.object`',
  ...
  '`ppp`', '(Marked) point pattern', '`?spatstat.geom::ppp`',
  '`~ g1/.../gm`', 'Nested grouping structure', '`?nlme::groupedData`; `?nlme::lme`',
  '', 'Quantile', '`?stats::quantile`',
  '`Surv`', 'Survival object', '`?survival::Surv`'
) |>
  matrix(nrow = 3L, dimnames = list(c('Term', 'Description', 'Reference'), NULL)) |>
  t.default() |>
  as.data.frame.matrix() |> 
  kable(caption = 'Terms used in this vignette')
```
...
## With one-and-only-one `ppp`-hypercolumn
...
In this section, we outline the batch process of spatial point pattern analyses applicable to the one-and-only-one `ppp`-hypercolumn of a `hyperframe`.  
...
Batch processes that return a `fv`-hypercolumn include
...
c(
  ...
  t.default() |>
  as.data.frame.matrix() |> 
  kable(caption = 'Batch process that returns `fv`-hypercolumn')
```


Batch processes that return a `numeric`-hypercolumn include
...
) |> kable(caption = 'Batch process that returns `numeric`-hypercolumn')
```


Following example shows that multiple batch processes may be applied to a `groupedHyperframe` in a pipeline (`|>`).
...
The returned `groupedHyperframe` has 
