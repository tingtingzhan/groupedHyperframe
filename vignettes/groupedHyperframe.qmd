...
The derived class `'vectorlist'`, i.e., a `listof` `vector`s, `inherits` from class `'anylist'`. The implementation of `'vectorlist'` is inspired by class `'solist'`.

The `S3` method dispatch `t.vectorlist()` transposes a `'vectorlist'` of equi-`length`.

The motivation of the derived class `'vectorlist'` and the method dispatch `t.vectorlist()` is that function `spatstat.geom::with.hyperframe()` could be slow in a batch process.
