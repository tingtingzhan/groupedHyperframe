...
As we have mentioned in the previous section *Batch processes*, a same user-specified argument of `r` will be used for *all* `ppp.object`s in the `ppp`-hypercolumn. Suppose a naive user uses an $r$-vector well beyond the *recommended range* and/or *available range*. In this case, function `aggregate_fv()` prints a `message` of *Legal* $r_\text{max}$, which is the last value of $r$, that no value of `0`, `NaN` and/or `Inf` appears in *any* of the `fv`-returns, e.g., in the hypercolumn *`hladr.E`*.
