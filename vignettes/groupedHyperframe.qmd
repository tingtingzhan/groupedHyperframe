...
& = \left(\frac{f(x_0)}{2}+\sum _{k=1}^{N-1}f(x_{k})+\frac{f(x_N)}{2}\right)\Delta x, \quad\text{if}\ \forall k, \Delta x_{k}\equiv \Delta x
\end{split}
$${#eq-trapz}

We define the *average height* of a trapezoidal integration as (@eq-trapz) divided by $N\Delta x$, the length of $x$-domain,

$$
\dfrac{\displaystyle\int_{a}^{b}f(x)\,dx}{N\Delta x}\approx N^{-1}\left(\frac{f(x_0)}{2}+\sum _{k=1}^{N-1}f(x_{k})+\frac{f(x_N)}{2}\right)
$${#eq-vtrapz}

We illustrate the average height (@eq-vtrapz) of a trapezoidal integration using a toy example with $x_k$ and $y_k = f(x_k)$ below.
...
Function `vtrapz()` calculates the average height of a trapezoidal integration (@eq-vtrapz).  The (tentative) prefix `v` indicates "vertical".  The shaded rectangle in @fig-vtrapz has the same area as the trapezoidal integration.  Therefore, we define the height of the shaded rectangle as the *average height* of the trapezoidal integration.

```{r}
vtrapz(x, y)
```

In R nomenclature, the term "cumulative" indicates that *the `i`th element of the output is the operation result of the first-`i`-elements of the input*.  Function `pracma::cumtrapz()` calculates a cumulative trapezoidal integration. Note that the first element of the output is always `0`, as a trapezoidal integration needs at least $\left(x_0, f(x_0)\right)$ and $\left(x_1, f(x_1)\right)$. 

```{r}
pracma::cumtrapz(x, y)
```

Function `cumvtrapz()` calculates the cumulative average height of a trapezoidal integration.  Note that the first element of the output is always `NaN`, i.e., the result of `0` (trapzoidal integration) divided by `0` (length of $x$-domain).

```{r}
cumvtrapz(x, y)
```

@fig-vtrapz illustrates the concept of (cumulative) average height of a trapezoidal integration using our toy example.  [Say some English as a summary statement.]{style="background-color: #FFFF00"}

```{r}
#| label: fig-vtrapz
#| code-fold: true
#| code-summary: "R code to illustrate (cumulative) average height of trapezoidal integration"  
#| fig-height: 4
#| fig-align: left
#| fig-cap: "(Cumulative) average height of trapezoidal integration"
