...
Each one of the numeric-hypercolumns contains tabulated values on the common grid of $r$. One "slice" of the hypercolumn *`hladr.E.cumvtrapz`* at $r=50$ may be extracted by calling the `S3` method dispatch `spatstat.geom::with.hyperframe()` (which behaves *very differently* from function `base::with.default()`, see Appendix @sec-hyperframe),
...
Advanced users may use the `S3` method dispatch `t.vectorlist()` in Appendix @sec-vectorlist.
...
| Topic | Where to Find |
|-----|-----|
| `groupedHyperframe` | @sec-groupedHyperframe |
| $k$-means clustering | @sec-kmeans |
