...
-   the $(i+1)$-th element $i^{-1}(f(x_0)/2+\sum_{k=1}^{i-1}f(x_k)+f(x_i)/2)$. Note that the R `vector` indices start at `1L` instead of `0L`; see further about [zero-based numbering](https://en.wikipedia.org/wiki/Zero-based_numbering)
