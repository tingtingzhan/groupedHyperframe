...
c(
  ...
  '`hypercolumn`, `hyperframe`', '(Column of) hyper data frame', '`spatstat.geom::hyperframe`',
...
### From a `data.frame`
The S3 method dispatch `as.groupedHyperframe.data.frame()` inspects the input data by the (nested) grouping structure, identifies the column(s) which are not identical within the lowest group, and converts them into `hypercolumns`.  Data set `Ki67` in this package has non-identical column `Ki67` in the nested grouping structure, thus column `Ki67` is converted into a `hypercolumn`.

```{r}
Ki67 |> as.groupedHyperframe(group = ~ patientID/tissueID, mc.cores = 1L)
```
